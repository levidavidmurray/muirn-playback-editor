<template>
  <div class="max-w-screen-lg mx-auto">
    ...
    <div class="bg-secondary-100 rounded-lg overflow-hidden py-4 px-4 mt-6">
      ...
      >
        <!-- Time Indicator -->
        <div class="flex justify-between w-100 mb-4 text-sm font-semibold select-none">
        ...
        <div class="relative flex w-100">
          ...
          >
            <span class="select-none" style="font-size: 11px">{{ segment.endTime - segment.startTime }}s</span>
          </div>
          <!-- Timeline Scrubber -->
          <div ref="scrubber" class="scrubber z-20" :class="{ 'pointer-events-none': isMovingScrubber }">
            <div
...
let segments: VideoSegment[] = reactive([])
let segmentElRefs: Ref<HTMLElement[]> = ref([])
...
function onTimeUpdate(event: any) {
  ...
  currentSegmentIndex = findSegmentIndexForTime(currentTime)
