  ...
  <base-template>
    <div class="max-w-screen-lg mx-auto">
      ...
      <div v-else>
        ...
        <video-player v-if="videoSrc" :video-src="videoSrc" type="application/x-mpegURL" />
      </div>
    </div>
  </base-template>
</template>
...
import { useVideoMutation, useVideoQuery } from '@/stores/query';
...
const { isLoading, data: videoData, error } = useVideoQuery(videoId)

const { mutate: updateVideo } = useVideoMutation(videoId)

const videoSrc = computed(() => {
  if (!videoData.value) return null
  return videoData.value.attributes?.manifestUrl
})
...
watch(videoData, (newUploadData) => {
  if (!newUploadData) return
  console.log('newUploadData', newUploadData)
  newVideoData.value = {
    title: newUploadData.attributes?.title || newVideoData.value.title,
    date: newUploadData.attributes?.date || newVideoData.value.date,
  }
})
